...
export class UpdateAssessmentUseCase {
    ...
    async execute(id: string, data: UpdateAssessmentDTO, file?: Express.Multer.File, tenantId?: string) {
        const existing = await this.repository.findById(id);
        if (!existing) throw new AppError('Assessment not found', 'NOT_FOUND', 404);
            ...
            if (existing.sebConfigFilePath) {
                ...
                if (oldKey) {
                    await this.storageProvider.delete(oldKey).catch((err) =>
                        console.error("Failed to delete old SEB file on disabling strictMode:", err)
                    );
                }
            }
        ...
        if (newStrictMode) {
            ...
            if (file) {
                ...
                const uploadResult: UploadFileResult = await this.storageProvider.upload({
                    key: r2Key,
                    buffer: file.buffer,
                ...
                try {
                    ...
                    if (existing.sebConfigFilePath && existing.sebConfigFilePath !== uploadResult.url) {
                        ...
                        if (oldKey && oldKey !== uploadResult.key) {
                            await this.storageProvider.delete(oldKey).catch((err) =>
                                console.error("Failed to delete previous SEB file on assessment update:", err)
                            );
                        }
                    }
                    ...
                    await this.storageProvider.delete(uploadResult.key).catch((err) =>
                        console.error("Failed to delete new R2 file on update failure:", err)
                    );
                    throw dbError;
                }
            } else {
