...
export class GetStudentCourseGradesUseCase {
    ...
    private extractFindings(aiFeedback: any, testOutput: any): StudentEvaluationFinding[] {
        const findings: StudentEvaluationFinding[] = [];
    ...
    private extractAiFeedback(aiFeedback: any): string | null {
        if (!aiFeedback) return null;
        if (typeof aiFeedback === 'string') return aiFeedback.trim() || null;
        if (typeof aiFeedback === 'object') {
            if (typeof aiFeedback.aiFeedback === 'string' && aiFeedback.aiFeedback.trim()) return aiFeedback.aiFeedback.trim();
            if (typeof aiFeedback.feedback === 'string' && aiFeedback.feedback.trim()) return aiFeedback.feedback.trim();
            if (typeof aiFeedback.comments === 'string' && aiFeedback.comments.trim()) return aiFeedback.comments.trim();
            if (typeof aiFeedback.teacherComments === 'string' && aiFeedback.teacherComments.trim()) return aiFeedback.teacherComments.trim();
        }
        return null;
    }

    async execute(userId: string, offeringId: string): Promise<StudentCourseGradesResponseDTO> {
        ...
        const gradeRecordMap = new Map<string, typeof gradeRecords[0]>();
        ...
        const grades: StudentAssessmentGradeItemDTO[] = assessments.map((assessment) => {
            ...
            let submissionStatus: 'NOT_STARTED' | 'IN_PROGRESS' | 'SUBMITTED' | 'EVALUATED' | 'FLAGGED' = 'NOT_STARTED';
            let score: number | null = null;
            let equivalentPoints: number | null = null;
            let percentage: number | null = null;
            let aiFeedback: string | null = null;
            let evaluationFindings: StudentEvaluationFinding[] = [];
