...
describe('GetStudentCourseGradesUseCase calculations and logic', () => {
    test('extractFindings handles findings and mistakes arrays correctly', () => {
        const useCase = new GetStudentCourseGradesUseCase({} as any);
        ...
        const findings = (useCase as any).extractFindings(aiFeedback, testOutput);
    ...
    test('extractAiFeedback correctly extracts string or object aiFeedback', () => {
        ...
        assert.equal((useCase as any).extractAiFeedback('Great work!'), 'Great work!');
        assert.equal((useCase as any).extractAiFeedback({ aiFeedback: 'Solid implementation.' }), 'Solid implementation.');
        assert.equal((useCase as any).extractAiFeedback({ feedback: 'Good logic.' }), 'Good logic.');
        assert.equal((useCase as any).extractAiFeedback(null), null);
        assert.equal((useCase as any).extractAiFeedback(''), null);
    });
