...
export class SocraticChatUseCase {
  ...
  async execute(params: {
    ...
    signal?: AbortSignal;
  }): Promise<{ updatedHistory: ChatMessage[]; assistantResponse: string }> {
    ...
    const finalWorkspaceContext: WorkspaceContext = {
      ...workspaceContext,
    ...
    const existingDbHistory = (submission.chatHistory as unknown as ChatMessage[]) || [];
    const currentHistory: ChatMessage[] = Array.isArray(existingDbHistory) && existingDbHistory.length > 0
      ? existingDbHistory
      : clientChatHistory || [];

    // 6. Notify real-time listeners that student asked a question
    const temporaryHistory: ChatMessage[] = [
      ...currentHistory,
