...
// ── Mock Helpers ────────────────────────────────────────────────────────────
function createMockGeminiService(responseResolver) {
    return {
        generateText: async (systemInstruction, prompt, jsonMode) => {
...
function createMockSubmission(overrides = {}) {
    return new SubmissionEntity(overrides.id || 'sub-123', overrides.assessmentId || 'assess-100', overrides.studentId || 'student-profile-1', overrides.status || 'IN_PROGRESS', overrides.startedAt || new Date(), overrides.submittedAt || null, 0, 0, overrides.codeSnapshot || { 'src/index.ts': 'console.log("hello world");' }, overrides.assessment || {
    ...
    }, overrides.student || { id: overrides.studentId || 'student-profile-1' }, null, null, overrides.totalScore || null, overrides.testOutput || null, overrides.aiFeedback || null, []);
}
function createMockRepo(initialSubmission) {
    ...
    const repo = {
        ...
        update: async (id, data) => {
            updates.push(data);
            current = new SubmissionEntity(current.id, current.assessmentId, current.studentId, data.status || current.status, current.startedAt, data.submittedAt !== undefined ? data.submittedAt : current.submittedAt, current.tabSwitchesCount, current.clipboardAttempts, data.codeSnapshot || current.codeSnapshot, current.assessment, current.student, current.testsPassedScore, current.aiQualityScore, data.totalScore !== undefined ? data.totalScore : current.totalScore, data.testOutput !== undefined ? data.testOutput : current.testOutput, data.aiFeedback !== undefined ? data.aiFeedback : current.aiFeedback, current.chatHistory);
            return current;
        },
        findByAssessment: async () => [current],
        updateGrade: async (id, totalScore, feedback) => {
...
describe('Automatic AI Assessment Grading Flow', () => {
    ...
    describe('FinishSubmissionUseCase Integration Flow', () => {
        test('Successful submission: RunCode succeeds, AI grades, score and feedback persisted as EVALUATED', async () => {
            ...
            let runCodeCalledWith = null;
            const mockRunCode = {
                execute: async (subId, uId, snapshot, entry) => {
...
//# sourceMappingURL=automatic-grading.test.js.map