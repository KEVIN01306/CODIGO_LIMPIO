 ...
 */
function resolveCommand(entryFile, language) {
...
export class RunCodeUseCase {
    ...
    async execute(submissionId, userId, codeSnapshot, entryFile) {
        // ── Authorization ─────────────────────────────────────────────────────
        ...
        if (submission.studentId !== studentProfile.id) {
            throw new AppError('You are not authorized to execute this submission', 'FORBIDDEN', 403);
        }
        ...
        // ── Execution ─────────────────────────────────────────────────────────
        const language = submission.assessment?.allowedLanguage || 'javascript';
        return this.runInSandbox(codeSnapshot, entryFile, language);
    }
    /** Writes files to a temp dir, executes, and cleans up. */
    async runInSandbox(codeSnapshot, entryFile, language) {
    ...
    spawnWithTimeout(cmd, args, cwd, absEntry) {
...
//# sourceMappingURL=run-code.usecase.js.map