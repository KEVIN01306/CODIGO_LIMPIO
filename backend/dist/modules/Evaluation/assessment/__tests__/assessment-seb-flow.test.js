...
async function runTests() {
    ...
    await assert.rejects(() => sebJwtProvider.verifyToken(tamperedToken), (err) => err.code === "INVALID_SEB_TOKEN", "Tampered token should be rejected");
    ...
    const resAssessment = resolveSebConfiguration({ sebConfigKey: "asm-key", sebConfigFilePath: "https://r2/asm.seb" }, { defaultSebConfigKey: "tenant-key", defaultSebConfigFilePath: "https://r2/tenant.seb" });
    ...
    const resTenant = resolveSebConfiguration({ sebConfigKey: null, sebConfigFilePath: null }, { defaultSebConfigKey: "tenant-key", defaultSebConfigFilePath: "https://r2/tenant.seb" });
    ...
    const resNone = resolveSebConfiguration({ sebConfigKey: null, sebConfigFilePath: null }, { defaultSebConfigKey: null, defaultSebConfigFilePath: null });
    ...
    const mockRepo = {
        ...
        create: async (data) => ({ id: "asm-new", ...data }),
    ...
    await assert.rejects(() => createUseCaseNoTenantSeb.execute({
        offeringId: "off-1",
        title: "Strict Exam without SEB",
        type: "EXAM",
        maxScore: 100,
        strictMode: true,
    }), (err) => err.code === "SEB_CONFIG_REQUIRED" && err.statusCode === 400, "Should require SEB config when tenant has no default");
    ...
    const mockSubmissionRepo = {
        findByAssessmentAndStudent: async (assessmentId, studentId) => {
    ...
    const startUseCase = new StartAssessmentUseCase(mockAssessmentRepo, mockSubmissionRepo, mockStudentRepo, mockEnrollmentRepo, mockTenantRepo, sebJwtProvider);
    ...
    await downloadController.downloadConfig({ query: { token: tokenInUrl } }, mockRes, (err) => { if (err)
        throw err; });
    ...
    assert.equal(sentBuffer.toString(), "fake-seb-content");
    ...
    let nextError = null;
    await downloadController.downloadConfig({ query: { token: mismatchedToken } }, mockRes, (err) => { nextError = err; });
...
//# sourceMappingURL=assessment-seb-flow.test.js.map