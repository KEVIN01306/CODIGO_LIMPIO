...
import { PrismaTenantRepository } from "../../../Tenant/infrastructure/prisma-tenant.repository.js";
import { PrismaStudentRepository } from "../../../Users/infrastructure/prisma-student.repository.js";
import { PrismaCourseEnrollmentsRepository } from "../../../Assignment/courseEnrollment/infrastructure/prisma-courseEnrollment.repository.js";
...
const startUseCase = new StartAssessmentUseCase(repository, submissionRepo, studentRepo, enrollmentRepo, tenantRepository, sebJwtProvider);
const controller = new AssessmentController(createUseCase, updateUseCase, getUseCase, listUseCase, deleteUseCase, startUseCase, tenantRepository, repository);
const router = Router();
const validatedMiddleware = new ValidatedMiddleware();
const authMiddleware = new AuthMiddleware();
router.use(authMiddleware.routeProtect);
router.post("/", authMiddleware.checkPermission(["assessments:create"]), MulterUploadProvider.seb("sebConfigFile", false), validatedMiddleware.validateBody(createAssessmentSchema), controller.create);
router.get("/", authMiddleware.checkPermission(["assessments:read"]), controller.list);
router.get("/seb/default-config", authMiddleware.checkPermissionSome(["assessments:create", "assessments:read", "assessments:update"]), controller.getDefaultSebConfig);
router.get("/:id", authMiddleware.checkPermission(["assessments:read"]), controller.getById);
router.post("/:id/start", authMiddleware.checkPermission(["assessments:read"]), controller.start);
router.put("/:id", authMiddleware.checkPermission(["assessments:update"]), MulterUploadProvider.seb("sebConfigFile", false), validatedMiddleware.validateBody(updateAssessmentSchema), controller.update);
router.delete("/:id", authMiddleware.checkPermission(["assessments:delete"]), controller.delete);
