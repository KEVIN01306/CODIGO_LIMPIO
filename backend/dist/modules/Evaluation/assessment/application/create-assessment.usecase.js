...
export class CreateAssessmentUseCase {
    ...
    async execute(data, file, tenantId) {
        ...
        if (hasFile || key) {
            ...
            try {
                ...
                await this.repository.delete(created.id).catch((err) => console.error("Failed to delete assessment after R2 upload error:", err));
                throw uploadError;
            }
            ...
            try {
                ...
                await this.storageProvider.delete(uploadResult.key).catch((err) => console.error("Failed to delete orphaned R2 file after DB update failure:", err));
                // Delete created assessment
                await this.repository.delete(created.id).catch((err) => console.error("Failed to delete assessment after DB update failure:", err));
                throw dbError;
            }
        }
