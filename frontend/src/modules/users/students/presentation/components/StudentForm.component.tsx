...
import { studentCreateSchema, studentUpdateSchema } from '../../domain/student.schema';
import type { StudentFormValues, StudentProfile } from '../../domain/student.interfaces';
import { createStudent, updateStudent } from '../../infrastructure/student.service';
...
const StudentForm: React.FC<StudentFormProps> = ({ initialData }) => {
  ...
  const schema = initialData ? studentUpdateSchema : studentCreateSchema;
  ...
  const onSubmit = async (values: StudentFormValues) => {
    ...
    try {
      ...
      setGlobalError(error.response?.data?.message || 'An error occurred while saving the student.');
    }
  };
      ...
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <TextField
            ...
            required
            fullWidth
            label="Student Number (Carné/Matrícula)"
            error={!!error}
            helperText={error?.message}
          />
        )}
